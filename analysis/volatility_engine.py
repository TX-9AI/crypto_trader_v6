...
class VolatilityEngine:
    ...
    def analyze(self, df_5m: pd.DataFrame,
        ...
        # ── VWAP Position ─────────────────────────────────────────────────────
        # VWAP must be anchored to today's session open (00:00 UTC).
        # Using the full df_5m lookback produces a rolling VWAP that drifts
        # and misrepresents where price is relative to today's fair value.
        try:
            import pandas as pd
            # Filter to today's candles only (UTC day boundary)
            if hasattr(df_5m.index, 'tz') and df_5m.index.tz is not None:
                today_start = pd.Timestamp.now(tz=df_5m.index.tz).normalize()
            else:
                today_start = pd.Timestamp.utcnow().normalize()
            df_today = df_5m[df_5m.index >= today_start]
            # Fall back to full df if today has fewer than 3 candles (session just started)
            if len(df_today) < 3:
                df_today = df_5m
            typical  = (df_today["high"] + df_today["low"] + df_today["close"]) / 3
            vwap_val = float((typical * df_today["volume"]).cumsum().iloc[-1] /
