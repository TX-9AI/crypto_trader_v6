...
# v2.0 — 2026-06-27 — Telegram, systemd env, GitHub token, single file
#         so exec bash activates venv in user's terminal correctly
#
# Run on a fresh EC2:
...
if [[ "$PAPER_INPUT" =~ ^[Nn] ]]; then
    ...
    print_ok "Cash: \$${CASH_BALANCE} → \$$(echo "$CASH_BALANCE * 10" | bc) buying power (auto-sized)"
fi
...
echo ""
echo -e "  Format: TX-9AI/crypto_trader_v6   (ENTER to skip)"
echo ""
GITHUB_REPO=""
GITHUB_TOKEN=""
